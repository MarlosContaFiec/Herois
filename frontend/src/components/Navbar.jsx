        ...
        <div className="hidden lg:flex items-center gap-1">
          {rotas.map((r) => (
            <Link
              key={r.path}
              to={r.path}
              className={location.pathname === r.path ? navLinkActive : navLink}
            >
              <r.icone size={16} />
              <span>{r.label}</span>
            </Link>
            ...
            <>
              <div className="text-right mr-2">
                ...
                <div className="text-xs text-amber-400 flex items-center gap-1">
                  <IconMoedas size={10} /> {usuario.moedas}
                </div>
              </div>
              <button
                onClick={logout}
                className={`${btnBase} ${btnGhost} text-xs`}
              >
                <IconSair size={14} /> Sair
              </button>
            </>
          )}
        </div>
          ...
          <div className="px-4 py-3 space-y-1">
            {rotas.map((r) => (
              <Link
                ...
                className={
                  location.pathname === r.path ? navLinkActive : navLink
                }
              >
                <r.icone size={16} />
                <span>{r.label}</span>
            ...
            {usuario && (
              <button
                onClick={logout}
                className={`${navLink} w-full text-left text-red-400`}
              >
                <IconSair size={16} />
                <span>Sair</span>
              </button>
