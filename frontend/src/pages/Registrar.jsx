...
export default function Registrar() {
  ...
  const handleSubmit = (e) => { e.preventDefault(); registrar.mutate({ nomeUsuario, email, senha }); };
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} className={`${cardBase} w-full max-w-md p-8`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" className={inputBase} value={nomeUsuario} onChange={(e) => setNomeUsuario(e.target.value)} placeholder="heroi123" required />
          </div>
          <div>
          ...
          <button type="submit" disabled={registrar.isPending} className={`${btnPrimary} w-full`}>{registrar.isPending ? 'Criando...' : 'Criar Conta'}</button>
        </form>
        <p className={`${textSecondary} text-center text-sm mt-6`}>Já tem conta? <Link to="/login" className="text-cyan-400 hover:text-cyan-300">Entrar</Link></p>
      </motion.div>
    </div>
  );
}