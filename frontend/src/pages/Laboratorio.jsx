  ...
  return (
    <AnimatedPage>
      <div className={`${pageHeader} flex items-center justify-between`}>
        <div>
          ...
          <p className={`${textSecondary} text-sm mt-1`}>
            Crie e gerencie cartas do seu laboratório pessoal (sandbox)
          </p>
        </div>
        <button
      ...
      {mostrarForm && (
        ...
        >
          ...
          >
            ...
            <div>
              <label className={labelBase}>Poder</label>
              <input
                type="number"
                className={inputBase}
                value={form.poder}
            ...
            <div className="col-span-2 md:col-span-4">
              <button
                ...
                className={btnPrimary}
              >
                {criar.isPending ? "Criando..." : "Criar no Laboratório"}
              </button>
            </div>
          </form>
        </motion.div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          icone={IconLaboratorio}
          titulo="Laboratório vazio"
          descricao="Crie cartas no seu laboratório pessoal"
        />
      ) : (
          ...
          {cartas.map((carta, i) => (
            <CartaCard key={carta.id} carta={carta} index={i}>
              <div className="flex gap-2">
                <button
                  onClick={() => transferir.mutate(carta.id)}
                  disabled={transferir.isPending}
                  className={`${btnAmber} ${btnSm} flex-1 flex items-center justify-center gap-1`}
                >
                  <IconEvoluir size={12} /> Transferir
                </button>
                <button
                  onClick={() => deletar.mutate(carta.id)}
                  disabled={deletar.isPending}
                  className={`${btnDanger} ${btnSm} flex items-center justify-center`}
                >
                  <IconDeletar size={12} />
                </button>
              </div>
