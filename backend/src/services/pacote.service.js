...
function ajustarPesosPorSorte(pesos, bonusSorte) {
  const total = pesos.COMUM + pesos.INCOMUM + pesos.RARA + pesos.EPICA + pesos.LENDARIA;
...
export async function comprar(usuarioId, pacoteId) {
  ...
  for (let i = 0; i < pacote.qtdCartas; i++) {
    ...
    const cartasDisponiveis = await pacoteRepo.listarCartasPorRaridade(raridade);

    if (cartasDisponiveis.length === 0) continue;

    const cartaSorteada = escolherCartaAleatoria(cartasDisponiveis);
    const existente = await pacoteRepo.encontrarCartaUsuario(usuarioId, cartaSorteada.id);
    ...
    if (existente) {
      ...
      cartasRecebidas.push({ tipo: 'DUPLICATA', carta: cartaSorteada, xpGanho });
    } else {
      await pacoteRepo.criarCartaUsuario({ usuarioId, cartaId: cartaSorteada.id });
      cartasRecebidas.push({ tipo: 'NOVA', carta: cartaSorteada });
    }
  }

  await usuarioRepo.atualizar(usuarioId, { moedas: usuario.moedas - pacote.custo });
  ...
  const pityRestante = Math.max(0, 100 - aberturas);

  return { cartasRecebidas, moedasRestantes: usuario.moedas - pacote.custo, aberturas, pityRestante };
}
...
export async function obterPity(usuarioId, pacoteId) {
  ...
  const cartasUnicas = await pacoteRepo.listarCartasPorRaridade('UNICA');
  const cartaUnica = cartasUnicas.find((c) => c.pacoteUnicoId === pacoteId);

  return { aberturas, pityRestante: Math.max(0, 100 - aberturas), resgatavel, cartaUnica };
}
...
export async function resgatarPity(usuarioId, pacoteId, cartaId) {
  ...
  const aberturas = await pacoteRepo.contarAberturas(usuarioId, pacoteId);
  if (aberturas < 100) throw new Error(`Pity insuficiente. Faltam ${100 - aberturas} aberturas`);
