...
export async function evoluir(usuarioId, cartaUsuarioId) {
  ...
  const raridadeAtual =
    ORDEM_RARIDADE[
      ORDEM_RARIDADE.indexOf(cu.carta.raridade) + cu.contagemEvoluida
    ];
  ...
  if (cu.nivel < 10) {
    throw new Error(
      `Carta precisa estar no nível 10 para evoluir. Nível atual: ${cu.nivel}`,
    );
  }
  ...
  if (usuario.moedas < custo)
    throw new Error(`Moedas insuficientes. Custo: ${custo}`);
  ...
  const novoPoder = Math.floor(
    novaFaixa.min + (novaFaixa.max - novaFaixa.min) * 0.5,
  );
