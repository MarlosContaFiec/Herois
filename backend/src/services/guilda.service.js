...
export async function criar(criadorId, dados) {
  ...
  if (usuario.moedas < 1000) throw new Error('Moedas insuficientes. Custo: 1000');
...
export async function entrar(usuarioId, guildaId) {
  ...
  if (guilda.tipoEntrada === 'PEDIDO') {
    const pedidoExistente = await guildaRepo.encontrarPedido(guildaId, usuarioId);
...
export async function sair(usuarioId) {
  ...
  if (membro.papel === 'LIDER') throw new Error('O líder não pode sair. Transfira a liderança primeiro');
...
export async function listarPedidos(usuarioId, guildaId) {
  ...
  if (membro.papel === 'MEMBRO') throw new Error('Apenas líder e vice podem ver pedidos');
...
export async function responderPedido(usuarioId, guildaId, pedidoId, aceitar) {
  const membro = await guildaRepo.encontrarMembro(usuarioId);
  if (!membro || membro.guildaId !== guildaId) throw new Error('Você não pertence a esta guilda');
  if (membro.papel === 'MEMBRO') throw new Error('Apenas líder e vice podem responder pedidos');

  const pedido = await guildaRepo.encontrarPedidoPorId(pedidoId);
  if (!pedido || pedido.guildaId !== guildaId) throw new Error('Pedido não encontrado');
  if (pedido.status !== 'PENDENTE') throw new Error('Pedido já foi processado');

  if (aceitar) {
    await guildaRepo.criarMembro({ guildaId, usuarioId: pedido.usuarioId, papel: 'MEMBRO' });
...
export async function convidar(usuarioId, guildaId, usuarioConvidadoId) {
  const membro = await guildaRepo.encontrarMembro(usuarioId);
  if (!membro || membro.guildaId !== guildaId) throw new Error('Você não pertence a esta guilda');
  if (membro.papel === 'MEMBRO') throw new Error('Apenas líder e vice podem convidar');
  ...
  await guildaRepo.criarMembro({ guildaId, usuarioId: usuarioConvidadoId, papel: 'MEMBRO' });
...
export async function expulsar(usuarioId, guildaId, usuarioExpulsoId) {
  const membro = await guildaRepo.encontrarMembro(usuarioId);
  if (!membro || membro.guildaId !== guildaId) throw new Error('Você não pertence a esta guilda');
  if (membro.papel === 'MEMBRO') throw new Error('Apenas líder e vice podem expulsar');

  const alvo = await guildaRepo.encontrarMembro(usuarioExpulsoId);
  if (!alvo || alvo.guildaId !== guildaId) throw new Error('Jogador não pertence a esta guilda');
  if (alvo.papel === 'LIDER') throw new Error('Não é possível expulsar o líder');
...
export async function promover(usuarioId, guildaId, alvoId, novoPapel) {
  const membro = await guildaRepo.encontrarMembro(usuarioId);
  if (!membro || membro.guildaId !== guildaId) throw new Error('Você não pertence a esta guilda');
  if (membro.papel !== 'LIDER') throw new Error('Apenas o líder pode promover/rebaixar');
...
export async function agendarTroca(usuarioId, guildaId, horarioFixo, manual) {
  ...
  if (membro.papel === 'VICE') {
    if (guilda.ultimoAgendadorId === guilda.criadorId && guilda.ultimoAgendamento) {
      const diffDias = Math.floor((new Date() - new Date(guilda.ultimoAgendamento)) / (1000 * 60 * 60 * 24));
      if (diffDias < 3) throw new Error('O líder definiu o horário há menos de 3 dias. Não é possível alterar');
    }
    if (horarioFixo) {
      await guildaRepo.atualizarGuilda(guildaId, {
        tipoAgendaTroca: 'FIXO',
...
export async function deletar(usuarioId) {
  const membro = await guildaRepo.encontrarMembro(usuarioId);
  if (!membro) throw new Error('Você não pertence a uma guilda');
  if (membro.papel !== 'LIDER') throw new Error('Apenas o líder pode deletar a guilda');
